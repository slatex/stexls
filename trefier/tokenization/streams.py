...
class LatexTokenStream:
    def __init__(
        ...
        """
        self.lower = lower
        # determine extra words by language
        extra = ''
        if lang in ('en', 'english'):
            # matches e.g. it's and don't
            extra = r"'s|n't|"
        elif lang in ('de', 'deutsch'):
            # matches allows for words with äüö and ß
            extra = r"([\w\d][\w\d_\-]*)?(\{\\ss\}|\\ss|\\?\"(a|A|o|O|u|U|s|S))+([\w\d][\w\d_\-]*)?|"
        # compile the word regex
        self.words = re.compile(extra + words)
        ...
        self._perform_german_replacements = perform_character_replacements and lang in ('de', 'deutsch')
    ...
    def from_file(
        ...
        lower: bool = True,
        lang: str = 'en',
        perform_character_replacements: bool = False) -> Optional[LatexTokenStream]:
