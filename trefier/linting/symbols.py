...
def _make_argument_count_mismatch_message(expected, found: int) -> str:
    return f"Expected were {expected} argument(s), but found {found}"
...
class ModuleDefinitonSymbol(Symbol):
    ...
    def from_node(node: Environment) -> ModuleDefinitonSymbol:
        ...
        if node.env_name != 'modsig':
            raise LinterException(f'Expected environment to have name "modsig" but found "{node.env_name}"')

        if len(node.rargs) < 1:
            raise LinterArgumentCountException.create("exactly 1", len(node.rargs))
        ...
        symbol = ModuleDefinitonSymbol(module_name, node.parser.file, mod_name_arg_range)
        if len(node.rargs) > 1:
            symbol.errors.append(_make_argument_count_mismatch_message(1, len(node.rargs)))
        return symbol
...
class ModuleBindingDefinitionSymbol(Symbol):
    ...
    def from_node(node: Environment) -> ModuleBindingDefinitionSymbol:
        ...
        if node.env_name != 'mhmodnl':
            raise LinterException(f'Expected environment to have name "mhmodnl" but found "{node.env_name}"')

        if len(node.rargs) < 2:
            raise LinterArgumentCountException.create("exactly 2", len(node.rargs))
        ...
        symbol = ModuleBindingDefinitionSymbol(module_rarg.text.strip(), binding_language, node.parser.file, module_range)
        if len(node.rargs) > 2:
            symbol.errors.append(_make_argument_count_mismatch_message(2, len(node.rargs)))
        return symbol
...
class EnvironmentSymbolWithStaticArgumentCount(Symbol):
    ...
    def __init__(self,
        ...
        if not match:
            self.errors.append(f'{self} Invalid environment name "{env_name}"')
        if env_name == 'symdef':
            self.is_alt = False
        else:
    ...
    def get_info(env: Environment) -> Tuple[str, List[Location], List[List[str]], List[str]]:
        ...
        if len(env.rargs) == 0:
            raise LinterArgumentCountException.create("at least 1 rarg", 0)
        ...
        if match.group(2) == 'sym' and is_alt:
            errors.append(f'{location} sym* environments must not be prefixed with "a" (found: {env.env_name})')
        if len(env.oargs) > 1:
            errors.append(_make_argument_count_mismatch_message("1 or 0", len(env.oargs)))
        if len(env.rargs) != static_argument_count:
            errors.append(_make_argument_count_mismatch_message(static_argument_count, len(env.rargs)))

        return symbol_name, symbol_name_locations, search_terms, errors
...
class DefiSymbol(EnvironmentSymbolWithStaticArgumentCount):
    ...
    def from_node(defi: Environment) -> DefiSymbol:
        symbol_name, symbol_name_locations, search_terms, errors = EnvironmentSymbolWithStaticArgumentCount.get_info(defi)
...
class TrefiSymbol(EnvironmentSymbolWithStaticArgumentCount):
    ...
    def from_node(trefi: Environment) -> TrefiSymbol:
        symbol_name, symbol_name_locations, search_terms, errors = EnvironmentSymbolWithStaticArgumentCount.get_info(trefi)
        ...
        if len(trefi.oargs) == 1:
            ...
            if len(parts) > 2 or not parts or not parts[-1] or not parts[-1][-1]:
                errors.append(
                    'Expected a trefi argument of the format'
        ...
        symbol = TrefiSymbol(target_module_identifier, target_module_location, target_symbol_location,
                           symbol_name, symbol_name_locations, search_terms,
