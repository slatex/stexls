...
class LatexParser(SmglomLatexParserListener):
    ...
    def subtoken_stream(
        ...
        perform_character_replacements: bool = False,
        token_filter_fn: Optional[Callable[[Iterable[Token]], List[bool]]] = None) -> Iterator[Token]:
        ...
        Keyword Arguments:
            ...
            :param stopwords: Regex that detects words on which a split should be performed.
            :param token_filter: Regex for filtering out unwanted tokens generated by splitting on stopwords and then interlacing them again.
            :param perform_character_replacements: If True, replaces latex characters with the correct characters depending on the language.
            :param token_filter_fn: Filter function applied to the raw parsed token stream and allows filtering of selected tokens by
                                    returning True on that token's position.
        """
        extra = ''
        ...
        tokens = list(self.root.tokens)
        if token_filter_fn:
            tokens = [
                token
                for token, do_filter
                in zip(tokens, token_filter_fn(tokens))
                if not do_filter
            ]
        for token in tokens:
